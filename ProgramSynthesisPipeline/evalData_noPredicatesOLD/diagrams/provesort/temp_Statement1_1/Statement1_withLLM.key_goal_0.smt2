...
(assert (distinct sort_Heap |sort_int[]| sort_java.lang.Object sort_int sort_Field sort_any sort_Null sort_boolean))
...
(assert (instanceof u_x_old sort_int))
...
; --- Sequent
(assert (not (=> (and (and (and (and (and (and (and (and (and (exists ((var_q Int)) (and (and (>= var_q 0) (< var_q (u2i (k_length u_A)))) (= (cast (k_select u_heap u_A (arr (i2u var_q))) sort_int) u_x))) (not (= u_A k_null))) (> (u2i (k_length u_A)) 0)) (< (u2i (k_length u_A)) 10)) (>= (u2i u_i) 0)) (< (u2i u_i) (u2i (k_length u_A)))) (exists ((var_q Int)) (and (and (>= var_q 0) (< var_q (u2i (k_length u_A)))) (= (cast (k_select u_heap u_A (arr (i2u var_q))) sort_int) u_x)))) (= (cast (k_select u_heap u_A |field_java.lang.Object::<created>|) sort_boolean) (b2u true))) (= (cast (k_select u_heap u_A_old |field_java.lang.Object::<created>|) sort_boolean) (b2u true))) (k_wellFormed u_heap)) (u2b (let ((u_heapAtPre u_heap) (u_x_old u_x) (u_A_old u_A)) unknown_0)))))

(check-sat)

; --- Translation of unknown values
; unknown_0 :  \<{  i = A.length - 1;}\> (and(and(and(exists{q:int}(and(and(geq(q,Z(0(#))),lt(q,length(A))),equals(int::select(heap,A,arr(q)),x))),equals(i,sub(length(A),Z(1(#))))),equals(x,x_old)),equals(A,A_old)))
