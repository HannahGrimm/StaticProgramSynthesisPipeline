...
def execute_sygus_pipeline(info):
    ...
    key_pre_returncode = run_key(key_file, split_out_pre, temp_folder)
    info["timestamps"].append(time.time())
    key_post_returncode = run_key(key_file, split_out_post, temp_folder)
    info["timestamps"].append(time.time())

    # Check if KeY runs were successful
    if key_pre_returncode != 0 or key_post_returncode != 0:
        print(f"Warning: KeY prover failed (pre: {key_pre_returncode}, post: {key_post_returncode})")
        # Write empty output files and return False to skip this test case
    ...
    try:
        ...
        if any([m for (n, m, t) in variables]):
            ...
            info["timestamps"].append(time.time())
            print("Start Sygus")
            # Default behavior: try full verification first; on failure retry without it.
            rc = run_cvc5(temp_folder + sygus_file, temp_folder + sygus_output_file)
            print("End Sygus")
            info["timestamps"].append(time.time())

            # If cvc5 failed, avoid calling the translator on empty output and
            # write fallback files for downstream steps. The runner now writes
            # stderr to `<output_file>.err` so we can inspect the reason.
            if rc != 0:
                err_file = temp_folder + sygus_output_file + ".err"
                print(f"cvc5 failed with return code {rc}. See {err_file} for details.")
                # Write empty java output and timing file so caller can continue
                java_file = "javaCode.txt"
                with open(temp_folder + java_file, "w") as output_file:
                    output_file.write(";")
