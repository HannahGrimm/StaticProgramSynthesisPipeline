...
def run_cvc5(input_file, output_file, timeout=100, full_verify=True):
    ...
    def try_run(args, desc):
        try:
            # Capture stdout to `output_file` and stderr to `output_file.err` for diagnostics
            err_path = output_file + ".err"
            with open(output_file, "w") as out_file, open(err_path, "w") as err_file:
                subprocess.run(
                    args,
                    stdout=out_file,
                    stderr=err_file,
