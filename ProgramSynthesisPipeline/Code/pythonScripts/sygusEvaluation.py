...
def main():
    src_dir = "C:\\Users\\hanna\\OneDrive\\Dokumente\\Masterarbeit\\ProgSynthProgram\\ProgramSynthesisPipeline\\evalData_noPredicates\\diagrams\\"
    projectsToEvaluate = [
         "maxElement",  # 1,2,3,5
         "LinearSearch",  # all
         "DutchFlag",  # 1,3
    ...
    for id, val in task_dict.items():
        ...
        if val["project"] == "maxElement" and x in []:
            continue
        if val["project"] == "LinearSearch" and x in []:
            continue
        if val["project"] == "DutchFlag" and x in []:
            continue
        if val["project"] == "sort" and x in []:
            continue
