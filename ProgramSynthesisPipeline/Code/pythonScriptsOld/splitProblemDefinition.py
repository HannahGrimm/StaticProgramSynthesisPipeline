...
def split_problem_definition(info, output_pre_filename, output_post_filename):
    ...
    input_content = re.sub(r'\\javaSource\s+"[^"]*"\s*\n?', '', input_content)
    ...
    # print("problem_parts", len(problem_parts))
    # print(problem_parts[1])
    problem_parts[1] = "{" + problem_parts[1]
    # print(problem_parts[1])

    pre_copy = input_content.replace(problem_part, problem_parts[0].strip() + "}")
    pre_copy += "\n\n\\proofScript \"macro 'simp-upd';\""

    post_copy = input_content.replace(
        problem_part, "\\problem {" + extract_post_condition(problem_parts[1])
    )
    post_copy += "\n\n\\proofScript \"macro 'simp-upd';\""
