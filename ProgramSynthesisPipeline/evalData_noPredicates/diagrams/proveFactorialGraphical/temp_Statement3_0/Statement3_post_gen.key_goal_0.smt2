...
(declare-fun u_f () U)
(declare-fun |u_Helper::factorial| (U U) U)
(declare-fun u_heap () U)
(declare-fun u_n () U)
(declare-const sort_Heap T)
(assert (distinct sort_boolean sort_int sort_Heap))
...
(assert (instanceof u_f sort_int))
(assert (forall ((var_0 U) (var_1 U)) (! (instanceof (|u_Helper::factorial| var_0 var_1) sort_int) :pattern ((|u_Helper::factorial| var_0 var_1)))))
(assert (instanceof u_heap sort_Heap))
(assert (instanceof u_n sort_int))

; --- Sequent
(assert (not (= u_f (|u_Helper::factorial| u_heap u_n))))
